...
function fakeMessage() {
    ...
    // recuperation de la question saisi par le l'utilisateur
    let msg_saisi_par_client = $('.message-input').val();

    let data = {
        'msg': msg_saisi_par_client
    };
    console.log(data)
    let url = "http://127.0.0.1:8080/api/chatbot/";
    //envoyer la requete au serveur via ajax
    $.ajax({
        url: url,
        type: 'GET', // "GET","POST","PUT","DELETE",
        data: data,
        dataType: 'json',
        success: function(resultatFromServer) {
            // la requete s'est bien deroule on a la reponse du serveur
            console.log(resultatFromServer);
            let bot = resultatFromServer;
            $('<div class="message new"><figure class="avatar"><img src="static/img/hakkillo.gif" /></figure>' + bot + '</div>').appendTo($('.mCSB_container'));
            setDate();
            updateScrollbar();
            i++;
    ...
    /* setTimeout(function() {
        ...
        $('<form method="POST"><div class="message new"><figure class="avatar"><img src="static/img/hakkillo.gif" /></figure>' + data + '</form> </div>').appendTo($('.mCSB_container'));
         setDate();
         updateScrollbar();
         i++;
     }, 1000 + Math.random() * 20 * 100);*/
